...
def game_loop(
    ...
    myPlayer = Player(data_file=myWriter.filename, gesture_list=gesture_list)
    ...
    while running:
        ...
        for event in events:
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_q:
                    set_current_gesture(None, 10)
                if event.key == pygame.K_w:
                    set_current_gesture(None, 11)
                if event.key == pygame.K_e:
                    set_current_gesture(None, 12)
                if event.key == pygame.K_r:
                    set_current_gesture(None, 13)
                if event.key == pygame.K_t:
                    set_current_gesture(None, 14)
                if event.key == pygame.K_y:
                    set_current_gesture(None, 15)
                if event.key == pygame.K_u:
                    set_current_gesture(None, 16)
                if event.key == pygame.K_i:
                    set_current_gesture(None, 17)
                if event.key == pygame.K_o:
                    set_current_gesture(None, 18)
                if event.key == pygame.K_p:
                    set_current_gesture(None, 19)
        ...
        if not playingback:
            ...
            if data:
                landmarks, gesture_label = data
                gesture_text = font.render(gesture_label, False, (255, 0, 0))
                window.blit(gesture_text, (window_width - window_width // 2, 40))
                print(gesture_label)
                myRenderHands.render_hands(landmarks)
