...
class GameFN:
    ...
    def restart_menu(self):
        ...
        self.render_words(
            "Game Over", 80, self.window_width / 2 - 10, self.window_height / 4, black
        )
        self.render_words(
            f"Score: {self.score}",
            100,
            self.window_width / 2 - 3,
            self.window_height / 2 - 7,
            green,
        )
        self.render_words(
    ...
    def render_words(self, input, size, x, y, color):
        ...
        surface = font.render(
            input, True, color
        )  # Render the text onto new surface with antialiasing
        rect = (
            surface.get_rect()
        )  # Get rectangular area of surface which text is drawn onto
        rect.midtop = (
            x,
            y,
        )  # Set middle-top coordinate of text rectangle for positioning
        self.window.blit(
            surface, rect
        )  # Draws text surface onto window at specified text rect position
    ...
    def make_throwables(self, throwable):
        ...
        self.stats[throwable] = {
            ...
            "dx/dt": random.randint(-4, 4),
            "dy/dt": random.randint(-11, -7),
            "time": 0,
        ...
        for key, value in self.stats.items():
            if not value["throwing"]:
                ...
                if value["y_pos"] > self.window_width:
                    ...
                    self.window.blit(
                        value["pic"], (int(value["x_pos"]), int(value["y_pos"]))
                    )
